...
extern user_access_ops_t uio_access;
extern user_access_ops_t vfio_access;
		    ...
		    void *buf, size_t len, off_t offset)
{
	...
	ret = dev->user_access_ops->read_config(dev, buf, len, offset);
		     ...
		     void *buf, size_t len, off_t offset)
{
	...
	if (dev == NULL || dev->user_access_ops == NULL) {
		ODP_DBG("Called pci_write_config with NULL device or dev->user_access_ops.\n");
		return -1;
	}

	if (dev->user_access_ops->read_config == NULL) {
		ODP_DBG("Called pci_write_config with write_config==NULL.\n");
		return -1;
	}

	ret = dev->user_access_ops->write_config(dev, buf, len, offset);
...
int pci_map_device(pci_dev_t *dev)
{
	...
	if (dev == NULL || dev->user_access_ops == NULL) {
		ODP_DBG("Called pci_map_device with NULL device or dev->user_access_ops.\n");
		return -1;
	}

	if (dev->user_access_ops->map_resource == NULL) {
		ODP_DBG("Called pci_map_device with map_resources==NULL.\n");
		return -1;
	}

	ret = dev->user_access_ops->map_resource(dev);
...
int pci_unmap_device(pci_dev_t *dev)
{
	...
	if (dev == NULL || dev->user_access_ops == NULL) {
		ODP_DBG("Called pci_unmap_device with NULL device or dev->user_access_ops.\n");
		return -1;
	}

	if (dev->user_access_ops->unmap_resource == NULL) {
		ODP_DBG("Called pci_unmap_device with unmap_resources==NULL.\n");
		return -1;
	}

	ret = dev->user_access_ops->unmap_resource(dev);
...
int pci_ioport_unmap(pci_dev_t *dev, pci_ioport_t *p)
{
	...
	if (dev->user_access_ops->ioport_unmap == NULL) {
		ODP_DBG("Called pci_unmap_device with pci_ioport_unmap==NULL.\n");
		return -1;
	}
...
void pci_ioport_read(pci_dev_t *dev, pci_ioport_t *p,
			...
			uint8_t devid, uint8_t function, pci_dev_t **devlist)
{
	...
	if (!ret) {
		if (!strcmp(driver, "vfio-pci"))
			...
			dev->user_access_ops = &uio_access;
			pci_map_device(dev);
		}
		else
...
static int pci_dump_scanned(void)
{
	...
	ODP_PRINT("list of scanned PCI devices:\n");
...
pci_dev_t* _odp_pci_lookup_by_addr(pci_addr_t* addr)
{
	...
	for (dev = devlist; dev != NULL; dev = dev->next)
	{
		if (pci_address_compare(&dev->addr, addr) == 0)
		{
			return dev;
		}
	}
...
int _odp_pci_init_global(void)
{
	...
	// required to allow in and out IO function calls
